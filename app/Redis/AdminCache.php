...
class AdminCache extends MasterCache
{
    private $lUser = LIST_ADMIN_USER;  // 用户 list
    private $hUser = HASH_ADMIN_USER_;  // 用户 hash
     ...
     * @author: Luoyan
     */
    public function adminUserCacheList($currentPage, $perPage)
    {
        // 判断当前 list 是否存在
        if (!$this->exists($this->lUser)) {
            // 创建 list 索引，并且判断创建结果
            if (!$this->createAdminUserListIndex()) return false;
        }
        // 获取数据的索引
        $index = $this->getPageLists($this->lUser, $perPage, $currentPage);
        if (!$index) return false;
        // 根据索引获取数据
        $users = $this->getUserHash($index);
        // 获取索引总条数
     ...
     * @author: Luoyan
     */
    public function getUserHash(array $indexs)
    {
        // 根据索引获取数据 （如果为空返回空数组）
     ...
     * @author: Luoyan
     */
    public function createAdminUserListIndex()
    {
        // 查询出所有用户的 id
        $userIds = AdminUser::fetchUserListsFor('id');
        // 判断查询结果
