...
class Analysis
{
    ...
    /**
     * @param $str string 要分词的字符串
     * @return array 分词后的unicode编码格式的数组
     */
    public function QuickCut($str)
    {
        //分析字符串  //开始分词  //分词结果
        $this->SetSource($str);
        $this->StartAnalysis();
        $keys = $this->GetFinallyResult();

        //处理成unicode码形式的数组
        $keysArr = explode(' ', ltrim($this->toUnicode($keys), ' '));

        return $keysArr;
    }

    /**
     * 将汉字转码成unicode，并去除中间的 \
     * @param $key string 要转码的关键字
     * @return string 转码后的结果
     */
    public function toUnicode($key)
    {
        return str_replace('\\', '', trim(json_encode($key), '"'));
    }
}